...
func TestConcurrentFileHashing(t *testing.T) {
	hasher := DefaultFileHasher{}
	numFiles := 10
	var files []string
	...
	defer removeTempFiles(t, files)

	var wg sync.WaitGroup
	results := make(chan [16]byte, numFiles)
	...
	for _, file := range files {
		...
		go func(f string) {
			defer wg.Done()
			hash, err := hasher.ComputeFileHash(f)
			if err != nil {
				t.Errorf("Error hashing file %s: %v", f, err)
				return
			}
			results <- hash
		}(file)
	...
	if hashCount != numFiles {
		t.Errorf("Expected %d hashes, got %d", numFiles, hashCount)
	}
}
